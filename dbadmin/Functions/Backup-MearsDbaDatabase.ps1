Function Backup-MearsDbaDatabase
{
<#
.SYNOPSIS
...
#>
	...
	{		
		...
		ForEach ($Database in $databasecollection)
		{
            ...
            if($BackupSetName) {
                $backup.BackupSetName = $BackupSetName
            }

            if($EncryptionOptions -and $server.VersionMajor -gt 11) {
                if($EncryptionOptions.EncryptorType -eq "ServerCertificate" -and $Server.Databases["Master"].Certificates.Name -contains $EncryptionOptions.EncryptorName) {
                    $Encryption = New-Object Microsoft.SqlServer.Management.Smo.BackupEncryptionOptions
                    $Encryption.Algorithm = $EncryptionOptions.Algorithm.ToString()
                    $Encryption.EncryptorType = $EncryptionOptions.EncryptorType.ToString()
                    $Encryption.EncryptorName = $EncryptionOptions.EncryptorName.ToString()
                    $backup.EncryptionOption = $Encryption
                    ...
                    Write-Warning "$FunctionName - Encryption options set but could not find a valid certificate with the name $($EncryptionOptions.EncryptorName)"
                }
            }
            else {
                Write-Warning "$FunctionName - Encryption is not supported with this version/edition of Sql Server"
            }
			...
			else
			{
			    ...
			    $BackupFileName = "$Prefix$($dbname)_$timestamp"
