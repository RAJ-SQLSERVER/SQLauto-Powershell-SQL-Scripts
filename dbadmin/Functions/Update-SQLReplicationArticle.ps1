function Update-SQLReplicationArticle {
  ...
  begin {
    ...
    Try {
        ...
        $srv = New-Object ('Microsoft.SqlServer.Management.Smo.Server') $publisherConn
    }
    Catch {
    ...
    If ($publication.IsExistingObject) {
        ...
        foreach ($table in $ArticleTable) {
            ...
            if ($article -eq $null) {
                ...
                if($ArticleColumns) {
                    #Check columns against publication table and only add ones that match local table
                    $columns = $srv.Databases[$publicationDbName].Tables[$object.ObjectName,$object.ObjectSchema].Columns | %{$_.Name}
                    [string[]]$AddCols = Compare-Object $columns $ArticleCols -IncludeEqual -ExcludeDifferent | %{$_.InputObject}
